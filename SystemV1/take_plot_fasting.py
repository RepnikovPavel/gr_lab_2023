...
J_flow_carb_vs1 = J_flow_prot_func.values
J_flow_prot_vs1 = J_flow_prot_func.values
J_flow_fat_vs1  = J_flow_fat_func.values
...
ax2 = ax.twinx()
y2 = INS_AUC_w_on_grid
y2[np.argwhere(y2==0.0)]=0.0

ax2.plot(time_sol,y2,c='r',linestyle='dashed',linewidth= 3)
ax2.set_ylabel(r'$AUC(INS,t-w,t)$'+ '\n'+ r'$ммоль \cdot мин$',color='r',fontsize = 15)
y2_ticks = np.arange(np.min(y2),np.max(y2)+200, 300)
...
ax3.plot(time_sol,T_a_on_grid,c='b',linestyle='dashed',linewidth= 3)
ax3.set_ylabel(r'$T_{fasting}$'+ '\n'+ r'$мин$',color='b',fontsize = 15)
y3_ticks = np.arange(np.min(T_a_on_grid),np.max(T_a_on_grid)+20, 40)
y3_labels = [latex_float(tick,0) for tick in y3_ticks]
ax3.set_yticks(y3_ticks)
ax3.set_yticklabels(y3_labels,color='b')
# ax3.tick_params(axis='both', which='major', labelsize=10)
ax3.set_ylim(y3_ticks[0],y3_ticks[-1])


fig.savefig(os.path.join(path_to_plots,'fasting.png'),bbox_inches = 'tight',dpi=1000)
plt.show()
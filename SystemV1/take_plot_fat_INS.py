...
y_ticks = np.arange(np.min(INS_vec),np.max(INS_vec)+1, 1)
...
ax2.set_ylabel(r'$J_{fat}^{+},ммоль \cdot мин^{-1}$',color='r',fontsize = 15)
y2_ticks = np.arange(np.min(y2),np.max(y2)+0.1, 0.1)
...
fig.savefig(os.path.join(path_to_plots,'INS_fat.png'),bbox_inches = 'tight',dpi=1000)
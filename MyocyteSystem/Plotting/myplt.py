...
def plot_lines(y_list, title):
    fig,ax = plt.subplots()
    fig.set_size_inches(16,9)
    for i in range(len(y_list)):
        ax.plot(np.arange(0, len(y_list[i])),y_list[i])
    ax.set_title(title)
    return fig,ax

def plot_lines_with_colors(y_list, title, colors,alpha = 0.3):
    fig,ax = plt.subplots()
    fig.set_size_inches(16,9)
    for i in range(len(y_list)):
        ax.plot(np.arange(0, len(y_list[i])),y_list[i],color=colors[i],alpha=alpha)
    ax.set_title(title)
    return fig,ax